...
public:
  ...
  TreeNode* sucscr(TreeNode *node1); //Finds successor
  TreeNode* nodemin(TreeNode *node1); //Finds minimum valued node in a given subtree rooted at node1
  int longestpath(TreeNode *node1); //Finds longest path of given node
  int shortestpath(TreeNode *node1); //Finds shortest path of given node
