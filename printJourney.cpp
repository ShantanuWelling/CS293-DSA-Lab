...
void Planner::printPlanJourneys(string srcStnName, string destStnName, int maxStopOvers, int maxTransitTime)
{
  ...
  listOfObjects<int> *tt=nullptr;
  listOfObjects<TrainInfoPerStation *> bfsq=nullptr;
  listOfObjects<int> *stops=nullptr;
  ...
  listOfObjects<queuenode> bfs=nullptr;
  ...
  while(true){//Iterate over adjacent stations from source station (level 1)
    ...
    while(true){
      ...
      if(bfsq==nullptr){
        bfsq= new listOfObjects<TrainInfoPerStation *>(trainitr->object);
      }
      else{
        listOfObjects<TrainInfoPerStation *> *temp=bfsq;
        bfsq=bfsq->next;
        bfsq=new listOfObjects<TrainInfoPerStation*>(trainitr->object);
        bfsq->prev=temp;
        temp->next=bfsq;
      ...
      if(tt==nullptr){
        tt=new listOfObjects<int>(0);
      }
      else{
        listOfObjects<int> *temp=tt;
        tt=tt->next;
        tt=new listOfObjects<int>(0);
        tt->prev=temp;
        temp->next=tt;
      ...
      if(stops==nullptr){
        stops=new listOfObjects<int>(maxStopOvers);
      }
      else{
        listOfObjects<int> *temp =stops;
        stops=stops->next;
        stops=new listOfObjects<int>(maxStopOvers);
        stops->prev=temp;
        temp->next=stops;
  ...
  listOfObjects<int> * currstop= stops; //Pointer to last element of journeycode BFS queue
...
public:
  int currday;
  TrainInfoPerStation* stninfo;
  StationConnectionInfo* prevstn, currstn, nextstn;
  int startday;
  listOfObjects<node> stoplist;//
  int numstops; //
  listOfObjects<int> transtime;//
  queuenode(int cd, TrainInfoPerStation* stn, StationConnectionInfo* pre, StationConnectionInfo* cur, StationConnectionInfo* nex, int sd){
    currday=cd;
    stninfo=stn;
    prevstn=pre;
    currstn=cur;
...
class node{
  ...
  string last;
  int jourc;
  printnode(string a, string b, int c){
    first=a;
    last=b;
    jourc=c;
  }
}
//currday, crrstn, nextstn, traininfo, starttrain, startday, layover list->stname string, day1 day2, t1 t2, jc1 jc2
